...
def _hash_assessment(a) -> str:
    parts = [
        str(a.total_score),
        a.quality_status.value,
        str(a.source_reliability_score),
        str(a.entity_resolution_score),
        str(a.identity_completeness_score),
        str(a.cross_source_consistency_score),
        str(a.freshness_score),
        "|".join(a.hard_blockers),
    ]
...
class TestASingleHighQualityProvider:
    ...
    def test_usable_not_rejected_for_single_source(self):
        a = SCORER.assess(
            ...
            arrival_date_value=date(2026, 6, 15),
            now=date(2026, 7, 1),
        )
...
class TestHEntityManualConfirmed:
    ...
    def test_manual_confirm_full_score(self):
        a = SCORER.assess(
            provider_names=("fake",),
            entity_match_status="manually_confirmed",
            has_house_bol=True,
            has_importer=True,
        )
...
class TestJOldData:
    ...
    def test_old_data_low_freshness(self):
        a = SCORER.assess(
            ...
            has_house_bol=True,
            has_importer=True,
            arrival_date_value=date(2020, 1, 1),
            now=date(2026, 7, 1),
        )
...
class TestPDateWithinTolerance:
    ...
    def test_date_within_year_fresh(self):
        a = SCORER.assess(
            ...
            has_house_bol=True,
            has_importer=True,
            arrival_date_value=date(2026, 1, 15),
            now=date(2026, 7, 1),
        )
...
class TestQDateOutsideTolerance:
    ...
    def test_old_date_lower_freshness(self):
        a = SCORER.assess(
            ...
            has_house_bol=True,
            has_importer=True,
            arrival_date_value=date(2022, 6, 15),
            now=date(2026, 7, 1),
        )
...
class TestRFutureDate:
    ...
    def test_future_date_blocked(self):
        a = SCORER.assess(
            ...
            has_house_bol=True,
            has_importer=True,
            arrival_date_value=date(2030, 1, 1),
            now=date(2026, 7, 1),
        )
...
class TestCorrectiveQualityFingerprint:
    def test_fingerprint_is_provider_order_and_database_id_independent(self):
        first = SCORER.assess(
    ...
    def test_shipment_or_quality_input_changes_fingerprint(self):
        base = SCORER.assess(
            shipment_fingerprint="a" * 64,
            provider_names=("fake",),
            entity_match_status="auto_match",
        ...
        shipment_changed = SCORER.assess(
            ...
            len(
                {
                    ...
                    shipment_changed.input_fingerprint,
                    quality_changed.input_fingerprint,
                }
            )
            == 3
        )

    def test_future_date_is_rejected_with_blocker(self):
        assessment = SCORER.assess(
            ...
            has_house_bol=True,
            has_importer=True,
            arrival_date_value=date(2026, 7, 2),
            now=date(2026, 7, 1),
        )
        assert assessment.quality_status == QualityStatus.REJECTED
        assert "impossible_future_date" in assessment.hard_blockers
