...
class TestFingerprintNoProvider:
    ...
    def test_different_provider_same_shipment_merges(self):
        s1 = NormalizedShipment(house_bol="HBOL-X", importer_name="Acme Inc",
                                carrier_scac="MAEU", arrival_date="2026-06-15",
                                provider="fake")
        s2 = NormalizedShipment(house_bol="HBOL-X", importer_name="Acme Inc",
                                carrier_scac="MAEU", arrival_date="2026-06-15",
                                provider="importyeti")
        assert s1.shipment_fingerprint == s2.shipment_fingerprint
...
class TestN1CrossProviderSameShipment:
    ...
    def test_same_fingerprint_across_providers(self):
        s_fake = NormalizedShipment(
            house_bol="MBOL-ABC", importer_name="Pacific Home Goods",
            carrier_scac="MAEU", provider="fake",
        )
        s_csv = NormalizedShipment(
            house_bol="MBOL-ABC", importer_name="Pacific Home Goods",
            carrier_scac="MAEU", provider="csv",
        )
        assert s_fake.shipment_fingerprint == s_csv.shipment_fingerprint

    def test_both_providers_traceable(self):
        s = NormalizedShipment(
            house_bol="MBOL-ABC", importer_name="PHG", provider="fake",
            raw_record_ids=(),
        )
...
class TestN2DifferentImporterSameBOL:
    ...
    def test_conflicting_importer_not_merged(self):
        s1 = NormalizedShipment(house_bol="HBOL-XX", importer_name="Acme Inc",
                                carrier_scac="MAEU", provider="fake")
        s2 = NormalizedShipment(house_bol="HBOL-XX", importer_name="Beta Corp",
                                carrier_scac="MAEU", provider="csv")
        assert s1.shipment_fingerprint != s2.shipment_fingerprint
...
class TestN3NameVariantsAcrossProviders:
    ...
    def test_name_variants_same_underlying(self):
        s1 = NormalizedShipment(house_bol="HBOL-N3", importer_name="ACME HARDWARE INC.",
                                provider="fake")
        s2 = NormalizedShipment(house_bol="HBOL-N3", importer_name="ACME HARDWARE, INC",
                                provider="csv")
        assert s1.shipment_fingerprint == s2.shipment_fingerprint
...
class TestN4MissingContainerAcrossProviders:
    ...
    def test_container_preserved_when_one_source_empty(self):
        s1 = NormalizedShipment(
            house_bol="HBOL-N4", importer_name="Acme",
            container_numbers=("TCLU1", "TCLU2"), provider="fake",
        )
        s2 = NormalizedShipment(
            house_bol="HBOL-N4", importer_name="Acme",
            container_numbers=(), provider="csv",
        )
        assert s1.shipment_fingerprint == s2.shipment_fingerprint
    ...
    def test_null_does_not_overwrite_non_null(self):
        full = NormalizedShipment(
            house_bol="HBOL-N4", importer_name="Acme",
            container_numbers=("TCLU1",), provider="fake",
        )
...
class TestN5IdempotentTwoRounds:
    ...
    def test_round1_and_round2_same_result(self):
        shipments_r1 = [
            NormalizedShipment(house_bol=f"R1-{i}", importer_name="Acme",
    ...
    def test_reverse_order_same_hash(self):
        forward = [
            NormalizedShipment(house_bol=f"R-{i}", importer_name="Acme",
                               provider="fake") for i in range(5)
        ]
        reverse = [
            NormalizedShipment(house_bol=f"R-{i}", importer_name="Acme",
                               provider="fake") for i in range(4, -1, -1)
        ]
...
class TestIdempotencyStats:
    ...
    def test_round2_creates_nothing_new(self):
        round1 = [NormalizedShipment(house_bol=f"ID-{i}", importer_name="TestCo",
                                     provider="fake") for i in range(3)]
        round2 = [NormalizedShipment(house_bol=f"ID-{i}", importer_name="TestCo",
                                     provider="fake") for i in range(3)]
...
class TestProviderInFingerprint:
    ...
    def test_fingerprint_independent_of_provider(self):
        ...
        for provider in ("fake", "csv", "importyeti", "datamyne"):
            s = NormalizedShipment(
                house_bol="HBOL-TEST", importer_name="TestCo",
                carrier_scac="MAEU", provider=provider,
            )
...
class TestProvidenceTracking:
    ...
    def test_multiple_raw_record_ids_tracked(self):
        ...
        s2 = NormalizedShipment(house_bol="HBOL-PRV", importer_name="Acme",
                                raw_record_ids=("id-1", "id-2"))
